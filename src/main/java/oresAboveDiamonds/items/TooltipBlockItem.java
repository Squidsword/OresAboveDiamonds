		...
		if(KeyboardUtil.isHoldingShift()) {
			...
			case "END AMETHYST ORE":
				...
				if(OADConfig.spawn_amethyst_end.get() == true) {
					...
					int veinsize =  (int) (OADConfig.amethyst_max_vein_size.get() / OADConfig.nether_vein_multiplier.get());
					tooltip.add(new StringTextComponent("\u00A7a" + "Generating!"));
					tooltip.add(new StringTextComponent("1 in " + spawnchance + " chance of spawning per chunk."));
					tooltip.add(new StringTextComponent("Max vein size of " + veinsize));
					tooltip.add(new StringTextComponent("Spawns below " + OADConfig.amethyst_max_spawn_height_nether.get()));
				} else {
					tooltip.add(new StringTextComponent("\u00A7c" + "DISABLED"));
					tooltip.add(new StringTextComponent("The generation for this ore can be enabled in the config file."));
				} */
			...
			case "NETHER BLACK OPAL ORE":
				if(OADConfig.spawn_black_opal_nether.get() == true) {
					...
					tooltip.add(new StringTextComponent("Spawns below " + OADConfig.black_opal_max_spawn_height_nether.get()));
				} else {
					tooltip.add(new StringTextComponent("\u00A7c" + "DISABLED"));
					tooltip.add(new StringTextComponent("Can be enabled in the config file."));
				}
			...
			case "END BLACK OPAL ORE":
				...
				if(OADConfig.spawn_black_opal_end.get() == true) {
					...
					tooltip.add(new StringTextComponent("\u00A7c" + "DISABLED"));
					tooltip.add(new StringTextComponent("You can enable generation of this ore in the config file."));
				}*/
				tooltip.add(new StringTextComponent("End ore generation isn't supported currently."));
				break;
			...
			case "END AMETHYST ORE":
				if(/*OADConfig.spawn_amethyst_end.get()*/ false == false ) {
					tooltip.add(new StringTextComponent("\u00A7c" + "DISABLED"));
				}
				break;
			...
			case "END BLACK OPAL ORE":
				if(/*OADConfig.spawn_black_opal_end.get()*/ false == false) {
					tooltip.add(new StringTextComponent("\u00A7c" + "DISABLED"));
				}
				break;
