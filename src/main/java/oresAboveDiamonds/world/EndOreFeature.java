...
//I found this file from ianm1647's open source More Progression mod. Many thanks to him.
/*
* Provided by kwpugh
* Pulled from Gobber
*/

public class EndOreFeature
  extends OreFeature
{
  public EndOreFeature(Function<Dynamic<?>, ? extends OreFeatureConfig> configFactoryIn) { super(configFactoryIn); }
  ...
  protected boolean func_207803_a(IWorld worldIn, Random random, OreFeatureConfig config, double p_207803_4_, double p_207803_6_, double p_207803_8_, double p_207803_10_, double p_207803_12_, double p_207803_14_, int p_207803_16_, int p_207803_17_, int p_207803_18_, int p_207803_19_, int p_207803_20_) {
    ...
    for (int j = 0; j < config.size; j++) {
      ...
      double d7 = ((MathHelper.sin(3.1415927F * f) + 1.0F) * d6 + 1.0D) / 2.0D;
      adouble[j * 4 + 0] = d0;
      adouble[j * 4 + 1] = d2;
      adouble[j * 4 + 2] = d4;
      adouble[j * 4 + 3] = d7;
                ...
                for (int j2 = l; j2 <= k1; j2++) {
                  ...
                  if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D) {
                    ...
                    if (!bitset.get(k2)) {
                      ...
                      if (worldIn.getBlockState((BlockPos)blockpos$mutable).getBlock() == Blocks.END_STONE) {
                        worldIn.setBlockState((BlockPos)blockpos$mutable, config.state, 2);
                        i++;
                      } 
                    } 
                  } 
